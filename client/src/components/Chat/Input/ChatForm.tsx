...
const ChatForm = memo(function ChatForm({
	...
	const hasStagedComposerContext = useRecoilCallback(
		({ snapshot }) =>
			(convoId: string) =>
				snapshot.getLoadable(store.pendingQuotesByConvoId(convoId)).getValue()
					.length > 0 ||
				snapshot.getLoadable(store.pendingManualSkillsByConvoId(convoId)).getValue()
					.length > 0,
		[],
	);
	...
	const {
		...
		onDuringRunModifier: steering.duringRunActive
			? handleDuringRunModifier
			: undefined,
	});
